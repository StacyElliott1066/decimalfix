...
function App() {
  ...
  const calculateRestHours = () => {
    ...
    if (previousActivities.length === 0 || targetActivities.length === 0) {
      console.log("⚠️ No valid activities for rest calculation.");
      return 0;
    }
    ...
    const lastEndTime = previousActivities.reduce((latest, activity) => {
      ...
      const postTime =
        (parseFloat(activity.prePost || "0") / 2) * 60 * 60 * 1000;
      const adjustedEnd = new Date(end.getTime() + postTime);
      return adjustedEnd > latest ? adjustedEnd : latest;
    }, new Date(0));
    ...
    const firstStartTime = targetActivities.reduce((earliest, activity) => {
      ...
      const preTime =
        (parseFloat(activity.prePost || "0") / 2) * 60 * 60 * 1000;
      const adjustedStart = new Date(start.getTime() - preTime);
      return adjustedStart < earliest ? adjustedStart : earliest;
    ...
    console.log("🕘 Previous adjusted end:", lastEndTime.toLocaleString());
    console.log("🕒 Target adjusted start:", firstStartTime.toLocaleString());
    console.log("⏱ Calculated rest hours:", restHours);

    return Math.max(0, restHours.toFixed(2));
  };
  ...
  console.log("📅 Target date is:", targetDate);
  console.log("⏱ Rest hours are:", restHours);
